...
export function generateStaticParams() {
  return telegramCommands.map(cmd => ({ slug: cmd.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const cmd = getCommandBySlug('telegram', params.slug);
  if (!cmd) return {};
  return {
    title: cmd.seoTitle,
    ...
    openGraph: {
      title: cmd.seoTitle,
      description: cmd.seoDescription,
      url: `https://www.botwave.online/commands/telegram/${cmd.slug}`,
      type: 'article',
    },
    alternates: { canonical: `/commands/telegram/${cmd.slug}` },
  };
}

export default function TelegramCommandPage({ params }: { params: { slug: string } }) {
  const cmd = getCommandBySlug('telegram', params.slug);
  if (!cmd) notFound();

  const related = cmd.relatedCommands
    .map(name => telegramCommands.find(c => c.name === name || c.slug === name))
    .filter(Boolean);

  const sameCat = getCommandsByCategory('telegram', cmd.category)
  ...
  const jsonLd = {
    ...
    '@type': 'HowTo',
    name: `How to use ${cmd.prefix}${cmd.name} on Telegram`,
      ...
      <div className="pt-32 pb-20 px-6">
        <div className="max-w-3xl mx-auto">
          <nav className="flex items-center gap-2 text-sm text-[var(--text-muted)] mb-8 flex-wrap">
            ...
            <span>/</span>
            <Link href="/commands/telegram" className="hover:text-[var(--primary)]">Telegram Bot</Link>
            <span>/</span>
            <span className="text-[var(--text-primary)]">{cmd.prefix}{cmd.name}</span>
          </nav>

          <div className="flex items-center gap-3 mb-6">
            <span className="px-3 py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-lg text-blue-500 text-xs font-semibold">Telegram Bot</span>
            ...
            <section className="mb-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[...related, ...sameCat].slice(0, 6).map(rc => rc && (
                  <Link key={rc.slug} href={`/commands/telegram/${rc.slug}`} className="p-4 bg-[var(--card-bg,var(--surface))] border border-[var(--border)] rounded-xl hover:border-blue-400 transition-colors">
          ...
          <div className="mt-12 p-8 bg-gradient-to-r from-blue-500/10 to-violet-500/10 border border-blue-500/20 rounded-2xl text-center">
            ...
            <p className="text-[var(--text-secondary)] mb-4">Create a bot via @BotFather, paste the token in BotWave, and go.</p>
