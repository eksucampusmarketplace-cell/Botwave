...
export const fixContent: Record<string, FixContent> = {
  'telegram-bot-disconnected': {
    ...
    symptoms: [
      ...
      '!help in Telegram gets no response.',
      'Telegram shows the linked device but says "Last seen: hours ago" on it.',
      'Auto-reconnect attempts in the dashboard log are failing in a row.',
    ],
    quickFix: [
      'Open Dashboard → Sessions → click the session → Reconnect.',
      'If reconnect fails immediately, check that the phone connected to the Telegram account is online (you can verify by sending yourself a message from Telegram Web).',
      'If the phone is offline, simply re-opening Telegram on the phone usually triggers BotWave to reconnect within 60 seconds.',
    ],
    causes: [
      {
        label: 'Phone offline / Telegram closed for too long',
        detail:
          'BotWave is a Linked Device, it depends on your phone\'s Telegram being awake periodically. If your phone is offline overnight or Telegram is force-closed, the linked device disconnects.',
        fix: [
          'Re-open Telegram on the phone.',
          'Wait up to 60 seconds for auto-reconnect.',
          'If still disconnected, click Reconnect in the dashboard.',
        ],
      },
      {
        label: 'Telegram Web logout (one-button kill)',
        detail:
          'If you tap "Log out from all devices" in Telegram Settings → Linked Devices, every Linked Device including BotWave is killed. The session must be re-paired from scratch.',
        fix: ['Open BotWave → Sessions → Reconnect with pairing code or QR.'],
      },
      {
        label: 'Linked-device cap (4 device limit)',
        detail:
          'Telegram allows 4 linked devices simultaneously. If you pair a 5th, Telegram silently kicks the oldest, which can include your BotWave session.',
        fix: [
          'In Telegram Settings → Linked Devices, remove old/unused entries.',
          'Re-pair BotWave.',
        ],
      },
      {
    ...
    resolutionSteps: [
      { title: 'Confirm the phone is online', body: 'Open Telegram on the device that owns the session and send yourself a message via Telegram → You → message yourself.' },
      ...
      { title: 'If still disconnected, check Telegram linked devices', body: 'Telegram → Settings → Linked Devices. If your BotWave entry is gone, re-pair.' },
    ...
    prevention: [
      'Avoid hitting "Log out from all devices" in Telegram.',
      'Keep your phone\'s Telegram open for at least a few seconds per day so the linked device stays warm.',
      'Don\'t exceed 4 linked devices.',
    ],
    relatedFix: ['telegram-bot-logged-out', 'bot-session-needs-reauth', 'telegram-multi-device-issue'],
    relatedHowTo: ['telegram-session-recovery', 'create-telegram-bot', 'telegram-pairing-code'],
    faqs: [
      { question: 'Will my custom commands and group settings be lost?', answer: 'No. Session-level config (commands, group rules, warnings, analytics) is stored against your BotWave account, not against the Telegram session credentials. Re-pairing keeps everything.' },
      { question: 'Why does the session disconnect every night?', answer: 'Phone going offline overnight is the most common cause. If this is consistent, leave the phone charging and Telegram open, or upgrade to a sessions-on-server plan (Boss) where the dependency on your phone is reduced.' },
      { question: 'How long can a session stay disconnected before something bad happens?', answer: 'Up to ~14 days. Beyond that, Telegram\'s Linked Device entry expires server-side and you must re-pair.' },
    ],
  },

  'telegram-bot-banned': {
    intro:
      'Seeing "Banned" status on a BotWave session means Telegram has blocked the underlying account at the server level, not a BotWave problem to fix, but there are concrete things to do next and very specific habits to avoid on the replacement session.',
    symptoms: [
      'Dashboard shows "Banned" status with timestamp.',
      'Telegram on the phone shows "Your phone number is banned from using Telegram" or similar.',
      '!help receives no reply and reconnect repeatedly fails.',
      'Trying to log into Telegram Web fails immediately.',
    ],
    quickFix: [
      'Do NOT pair the same number from a new device, Telegram will recognise the ban.',
      'Submit a ban appeal via Telegram → "Help" or by emailing support@telegram.com from the email tied to the account.',
    ...
    causes: [
      ...
      { label: 'Reports from recipients', detail: 'When multiple users tap "Report" in Telegram on messages from your number, the cumulative reports trigger a ban.', fix: ['Make sure messages add value; never spam.'] },
      { label: 'Brand-new number (< 24h)', detail: 'Telegram aggressively rate-limits and bans brand-new numbers attempting automation.', fix: ['Wait at least 7 days after registering a number before adding it to BotWave.'] },
    ],
    resolutionSteps: [
      { title: 'Confirm the ban', body: 'Try logging into Telegram itself on the phone. If you see the banned-number message, this is definitive.' },
      { title: 'Submit an appeal to Telegram', body: 'Inside the Telegram app on the phone, tap the ban dialog → "Request a Review". Or email support@telegram.com with subject "Banned account appeal" from the registered email.' },
    ...
    expectedResult: 'Either Telegram un-bans the original number (sometimes happens within 24-72h for legitimate cases) or you continue on a fresh number with safer practices.',
    prevention: [
    ...
    relatedFix: ['telegram-bot-disconnected', 'telegram-bot-logged-out', 'telegram-pairing-code-expired'],
    relatedHowTo: ['telegram-anti-ban-tips', 'telegram-anti-ban-setup', 'create-telegram-bot'],
    faqs: [
      { question: 'Will BotWave refund me if my number gets banned?', answer: 'Bans are imposed by Telegram and outside BotWave\'s control. We do not refund for bans, but we will help you re-set up on a new number quickly. The Terms of Service has the formal policy.' },
      ...
      { question: 'Can I use a virtual number?', answer: 'Telegram can detect and ban many virtual-number ranges (Google Voice, common VoIP providers). Use a real SIM where possible.' },
    ],
  },
  ...
  'telegram-qr-not-scanning': {
    ...
    resolutionSteps: [
      ...
      { title: 'Phone: Telegram → Settings → Linked Devices → Link a Device', body: 'Wait for the camera viewfinder.' },
  ...
  'telegram-auto-reply-not-working': {
    ...
    causes: [
      ...
      { label: 'Testing from the same account', detail: 'BotWave ignores messages from the same session that owns the AFK.', fix: ['Ask a friend to test, or use a second Telegram account.'] },
    ...
    relatedFix: ['telegram-ai-not-responding', 'telegram-bot-not-responding', 'telegram-bot-not-reading-messages'],
    relatedHowTo: ['auto-reply-telegram', 'telegram-ai-auto-reply', 'telegram-auto-responses'],
    faqs: [
  ...
  'telegram-bot-slow-response': {
    intro: 'Replies that take 10s+ when they used to take 1-2s. The bot is fine; somewhere between the bot and Telegram\'s servers is causing the delay.',
    ...
    causes: [
      ...
      { label: 'Telegram server slowness in your region', detail: 'Periodic regional slowdowns affect all Telegram-based services.', fix: ['Check Telegram itself, if regular Telegram messages are slow, it is upstream.'] },
    ...
    resolutionSteps: [
      { title: 'Run !ping in the chat', body: 'Returns the bot\'s latency to Telegram servers.', code: '!ping' },
    ...
    relatedFix: ['telegram-bot-slow', 'telegram-ai-not-responding'],
    relatedHowTo: ['telegram-anti-ban-setup', 'api-key-setup'],
    faqs: [
      { question: 'Why does BotWave intentionally delay replies?', answer: 'Bot-fast replies are a top Telegram ban signal. The 1.5-4.5s delay simulates human typing speed.' },
    ],
  },

  'telegram-bot-not-reading-messages': {
    ...
    quickFix: [
      ...
      'For Telegram, ensure the bot is admin if you need moderation actions.',
    ],
    causes: [
    ...
    relatedFix: ['telegram-bot-not-responding', 'telegram-bot-not-responding', 'telegram-group-bot-not-admin'],
    relatedHowTo: ['telegram-group-bot', 'telegram-bot-group', 'telegram-bot-permissions'],
    faqs: [
  ...
  'telegram-sticker-not-sending': {
    ...
    causes: [
      { label: 'HEIC images (Apple)', detail: 'Telegram\'s sticker pipeline mishandles HEIC.', fix: ['Convert to JPG before sending.'] },
    ...
    relatedFix: ['telegram-sticker-not-working', 'telegram-media-not-sending'],
  ...
  'telegram-bot-not-responding': {
    ...
    relatedFix: ['telegram-bot-not-responding', 'telegram-bot-no-permissions'],
  ...
  'telegram-download-not-working': {
    ...
    relatedFix: ['telegram-download-failed', 'telegram-media-not-sending'],
    relatedHowTo: ['download-tiktok-telegram', 'download-youtube-telegram', 'telegram-music-download'],
    faqs: [
      { question: 'Does BotWave cache downloaded videos?', answer: 'No, streamed through and discarded.' },
    ],
  },

  'telegram-bot-not-joining-group': {
    ...
    causes: [
      { label: 'Bot number not in contacts', detail: 'Telegram can only "Add Participant" from your contacts.', fix: ['Save the number as a contact first.'] },
      ...
      { label: 'Bot at Telegram\'s 4-device cap', detail: 'Rare but possible.', fix: ['Free up a device slot.'] },
      { label: 'Bot has Telegram privacy setting "Nobody can add me to groups"', detail: 'BotWave numbers should default to "Everyone".', fix: ['Phone → Settings → Privacy → Groups → Everyone.'] },
    ],
    resolutionSteps: [
    ...
    relatedFix: ['telegram-group-bot-not-admin', 'telegram-bot-not-responding'],
    relatedHowTo: ['telegram-group-bot'],
    faqs: [
      { question: 'Multiple groups at once?', answer: 'Yes, no per-session limit on group count beyond Telegram\'s own.' },
    ],
  },
  ...
  'telegram-ai-not-responding': {
    ...
    relatedFix: ['telegram-auto-reply-not-working', 'telegram-bot-slow-response'],
    relatedHowTo: ['telegram-ai-assistant', 'telegram-ai-auto-reply', 'api-key-setup'],
    faqs: [
  ...
  'telegram-bot-slow': {
    intro: 'Generic slowness, see telegram-bot-slow-response for a more detailed breakdown.',
    symptoms: ['Replies arrive in 10s+ when they used to be 1-2s.'],
    quickFix: ['!ping to measure latency.', 'Check Dashboard → Status.'],
    causes: [{ label: 'See telegram-bot-slow-response', detail: 'Same causes, this is a duplicate slug for SEO.', fix: ['See related.'] }],
    resolutionSteps: [{ title: 'See telegram-bot-slow-response', body: 'Detailed breakdown.' }],
    expectedResult: 'Acceptable response times.',
    relatedFix: ['telegram-bot-slow-response'],
    relatedHowTo: ['telegram-anti-ban-setup'],
  ...
  'telegram-sticker-not-working': {
    intro: 'Sticker creation fails entirely. See telegram-sticker-not-sending for detailed walkthrough.',
    ...
    resolutionSteps: [{ title: 'See telegram-sticker-not-sending', body: '' }],
    expectedResult: 'Stickers arrive correctly.',
    relatedFix: ['telegram-sticker-not-sending', 'telegram-media-not-sending'],
  ...
  'telegram-download-failed': {
    intro: '!download returns an error. See telegram-download-not-working for full causes; quick checklist here.',
    ...
    resolutionSteps: [{ title: 'See telegram-download-not-working', body: '' }],
    expectedResult: 'Media downloads.',
    relatedFix: ['telegram-download-not-working', 'telegram-media-not-sending'],
  ...
  'telegram-bot-403-error': {
    intro: 'Telegram error 403 = forbidden, usually a permission problem on the linked-device side. Most common cause: the Telegram account on the phone no longer authorises the Linked Device.',
    symptoms: ['Dashboard log shows "403 Forbidden" entries.', 'Bot disconnects shortly after.'],
    quickFix: ['Confirm Telegram → Linked Devices on the phone still lists the BotWave entry; if missing, re-pair.'],
    causes: [
      { label: 'Linked Device entry was removed', detail: 'Someone tapped Log Out on it.', fix: ['Re-pair.'] },
      { label: 'Account temporarily restricted', detail: 'Telegram pre-ban warning.', fix: ['Stop automation, message Telegram support, wait.'] },
    ],
    resolutionSteps: [
      { title: 'Verify Linked Device entry exists', body: 'Telegram → Settings → Linked Devices.' },
    ...
    relatedFix: ['telegram-bot-401-error', 'telegram-bot-disconnected', 'telegram-bot-logged-out'],
    relatedHowTo: ['telegram-session-recovery'],
  ...
  'telegram-bot-401-error': {
    intro: '401 = unauthorised, your stored session credentials no longer authenticate against Telegram. Always solvable by re-pairing.',
    symptoms: ['Dashboard log shows "401 Unauthorized".', 'Bot disconnects right after.'],
    quickFix: ['Re-pair with pairing-code flow.'],
    causes: [
      { label: 'Credentials rotated', detail: 'Telegram rotated session keys server-side.', fix: ['Re-pair.'] },
      { label: 'Account password-protected (rare)', detail: 'If Telegram enabled an extra security layer.', fix: ['Disable that or re-pair after configuring.'] },
    ],
    resolutionSteps: [
    ...
    relatedFix: ['telegram-bot-403-error', 'telegram-bot-disconnected', 'bot-session-needs-reauth'],
    relatedHowTo: ['telegram-session-recovery', 'telegram-pairing-code'],
  ...
  'telegram-bot-428-error': {
    ...
    causes: [
      ...
      { label: 'Telegram deploy', detail: 'Telegram\'s own infrastructure rolling.', fix: ['Self-resolves.'] },
    ],
    resolutionSteps: [
    ...
    relatedFix: ['telegram-bot-515-error', 'telegram-bot-disconnected'],
    relatedHowTo: ['telegram-session-recovery'],
  ...
  'telegram-pairing-code-expired': {
    ...
    symptoms: ['"Code expired" message in Telegram.', 'Telegram pairing screen rejects the code.'],
    ...
    resolutionSteps: [
      ...
      { title: 'Have Telegram pre-loaded on the pairing screen before clicking Generate', body: 'Pre-position so you can type the code immediately.' },
    ],
    expectedResult: 'Pairing succeeds within the 60s window.',
    relatedFix: ['telegram-qr-not-scanning', 'telegram-bot-disconnected'],
    relatedHowTo: ['telegram-pairing-code', 'create-telegram-bot'],
    faqs: [{ question: 'Why 60s?', answer: 'Telegram\'s security requirement.' }],
  },

  'telegram-session-conflict': {
    intro: 'Pairing a new device when 4 are already linked = Telegram silently disconnects the oldest. If that oldest was BotWave, your bot vanishes.',
    symptoms: ['Bot was working, suddenly disconnected after pairing a new device.', 'Telegram Linked Devices no longer shows BotWave.'],
    quickFix: ['Re-pair BotWave; remove one of the other linked devices in Telegram first.'],
    causes: [{ label: '4-device cap', detail: 'Telegram enforces.', fix: ['Drop a device.'] }],
    resolutionSteps: [
      { title: 'Telegram → Linked Devices', body: 'Remove unused entries.' },
      { title: 'Re-pair BotWave', body: '' },
    ],
    expectedResult: 'BotWave reclaims a slot and reconnects.',
    relatedFix: ['telegram-multi-device-issue', 'telegram-bot-disconnected'],
    relatedHowTo: ['telegram-pairing-code', 'create-telegram-bot'],
    faqs: [{ question: 'Will Telegram tell me which device was kicked?', answer: 'No, silent. Use the dashboard\'s connection log.' }],
  },

  'telegram-multi-device-issue': {
    intro: 'Telegram Multi-Device feature lets up to 4 devices be linked simultaneously, but BotWave occupies one slot. Awareness prevents accidentally kicking the bot.',
    ...
    resolutionSteps: [{ title: 'Audit linked devices', body: 'Telegram → Settings → Linked Devices.' }],
    expectedResult: 'BotWave stable.',
    relatedFix: ['telegram-session-conflict', 'telegram-bot-disconnected'],
    relatedHowTo: ['telegram-session-recovery'],
    faqs: [{ question: 'Can BotWave use 2 slots?', answer: 'No, one Linked Device entry per session.' }],
  },

  'telegram-group-bot-not-admin': {
    ...
    causes: [{ label: 'Not admin', detail: 'Telegram permission model.', fix: ['Promote.'] }],
    ...
    relatedFix: ['telegram-bot-not-reading-messages', 'bot-commands-not-working'],
    relatedHowTo: ['telegram-group-bot', 'telegram-moderation-setup', 'telegram-bot-permissions'],
  ...
  'telegram-media-not-sending': {
    ...
    causes: [
      { label: 'Telegram media servers slow', detail: 'Upstream.', fix: ['Wait.'] },
      { label: 'File exceeds Telegram limit', detail: 'Telegram caps: 100 MB video, 16 MB audio, 5 MB sticker.', fix: ['Resize/compress.'] },
    ...
    relatedFix: ['telegram-sticker-not-sending', 'telegram-download-failed'],
    relatedHowTo: ['telegram-sticker-maker', 'download-tiktok-telegram'],
    faqs: [{ question: 'Telegram file size limits?', answer: 'Video 100 MB, audio 16 MB, sticker 100 KB static / 500 KB animated.' }],
  },

  'telegram-bot-duplicate-messages': {
  ...
  'telegram-bot-wrong-language': {
    ...
    relatedHowTo: ['telegram-translate-messages'],
  ...
  'bot-commands-not-working': {
    ...
    relatedFix: ['telegram-bot-not-responding', 'telegram-bot-not-responding', 'telegram-group-bot-not-admin'],
    relatedHowTo: ['telegram-bot-permissions', 'set-up-bot-dashboard'],
  ...
  'bot-session-needs-reauth': {
    ...
    relatedFix: ['telegram-bot-401-error', 'telegram-bot-403-error', 'telegram-bot-logged-out'],
    relatedHowTo: ['telegram-session-recovery'],
    faqs: [{ question: 'Will I lose config?', answer: 'No.' }],
  },

  'telegram-bot-logged-out': {
    intro: '"Bot logged out" status means the Telegram Linked Device entry for the session has been removed, either by you or by Telegram itself.',
    symptoms: ['Status "Logged out".', 'Telegram → Linked Devices no longer shows BotWave.'],
    quickFix: ['Re-pair.'],
    causes: [
      { label: 'You tapped Log Out on the device in Telegram', detail: '', fix: ['Re-pair.'] },
      { label: 'Telegram removed it (>14 days idle)', detail: '', fix: ['Re-pair.'] },
    ...
    relatedFix: ['telegram-bot-disconnected', 'telegram-session-conflict'],
    relatedHowTo: ['telegram-session-recovery', 'telegram-pairing-code'],
