...
<table><tr><th>Agent</th><th>Runs</th><th>Passed</th><th>Failed</th><th>Avg Latency</th></tr>"""
    for agent, stats in sorted(traces_data.get("agent_stats", {}).items()):
        cls = "pass" if stats.get("failed", 0) == 0 else "fail"
        html += f"<tr><td>{agent}</td><td>{stats.get('total', 0)}</td><td class='{cls}'>{stats.get('passed', 0)}</td><td class='fail'>{stats.get('failed', 0)}</td><td>{stats.get('avg_latency_ms', 0) / 1000:.1f}s</td></tr>"
    html += """</table>
...
async def self_improvement_status(_: None = Depends(require_key)) -> dict[str, Any]:
    return {
        ...
        "self_tests": {"endpoint": "/self-tests/quality", "covers": ["web_search_parsing", "researcher_enrichment", "safe_healer_commands", "manager_capability_truth", "quality_policy"]},
    }
