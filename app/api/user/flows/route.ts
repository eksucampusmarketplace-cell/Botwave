...
import { getCachedFlows, cacheFlows, invalidateFlows, getCachedUserPlan, cacheUserPlan } from '@/lib/redisApiCache';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cached = await getCachedFlows(user.id);
    ...
    const result = data || [];
    await cacheFlows(user.id, result);
